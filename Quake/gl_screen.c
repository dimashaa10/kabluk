...
static GLuint FXAA_CreateShader_Simple(void)
{
	...
	const GLchar* fragSource =
		...
		"    vec4 nw = texture(u_tex, pos + vec2(-1.0, -1.0) * u_rcpFrame);\n"
		"    vec4 ne = texture(u_tex, pos + vec2(1.0, 1.0) * u_rcpFrame);\n"
		"    vec4 sw = texture(u_tex, pos + vec2(-1.0, 1.0) * u_rcpFrame);\n"
		"    vec4 se = texture(u_tex, pos + vec2(1.0, 1.0) * u_rcpFrame);\n"
		"    \n"
...
"\n"
        ...
        "  vec3 rgbNW = texture(u_tex, TexCoord.xy + vec2(-1.0,-1.0)*u_rcpFrame).rgb;\n"
        "  vec3 rgbNE = texture(u_tex, TexCoord.xy + vec2( 1.0,-1.0)*u_rcpFrame).rgb;\n"
        "  vec3 rgbSW = texture(u_tex, TexCoord.xy + vec2(-1.0, 1.0)*u_rcpFrame).rgb;\n"
        "  vec3 rgbSE = texture(u_tex, TexCoord.xy + vec2( 1.0, 1.0)*u_rcpFrame).rgb;\n"
		"  vec4 rgbaM = texture(u_tex, TexCoord.xy);\n"
		"  vec3 rgbM  = rgbaM.rgb;\n"
		"  float alpha = rgbaM.a;\n"
        "\n"
        ...
        "\n"
        "  vec3 rgbA = 0.5*(texture(u_tex, TexCoord.xy + dir*(1.0/3.0-0.5)).rgb +\n"
        "                   texture(u_tex, TexCoord.xy + dir*(2.0/3.0-0.5)).rgb);\n"
        "  vec3 rgbB = 0.25*(texture(u_tex, TexCoord.xy + dir*(-0.5)).rgb +\n"
        "                    texture(u_tex, TexCoord.xy + dir*( 0.5)).rgb) + 0.5*rgbA;\n"
        "  float lumaB = dot(rgbB,luma);\n"
		"  vec3 rgbOut = ((lumaB < lumaMin)||(lumaB > lumaMax)) ? rgbA : rgbB;\n"
		"  FragColor = vec4(rgbOut, alpha);\n"
        "}\n";
